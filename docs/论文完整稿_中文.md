...
（图 5、表 3）。成分分解（主 benchmark，图 6、表 5）：仅长度 [ ] → 工程特征 [ ] → 最优嵌入 [ ] → 融合 [ ]；
融合相对工程特征 +[ ]（p=[ ]）、相对单一嵌入 +[ ]（p=[ ]）。结论：**可解释序列属性与预训练表示包含部分
互补信息，融合优于任一单一视图——这是融合奏效的机制；门控权重仅反映模型更依赖哪类输入，不作机制
论断。**

![融合 vs k-mer](../results/figures/fig_fine_fusion_forest.png)

**图5. 融合 − k-mer（分组自助法 95% CI）。** 左 macro-AUPRC、右 macro-AUROC；四个设置的差值与显著性
（`*` / `ns`）。

![成分分解与门控](../results/figures/fig_fine_components.png)

**图6. 主 benchmark 的成分分解。** 仅长度 / 工程特征 / 最优基础模型嵌入 / 融合的 test macro 指标，附
...
整条 FM 视图的置换作为量级标尺（图 7）。重要性最高的若干特征为 [ ]（如 [motif/长度/组成] 通道），
...
**图7. 工程特征置换重要性（主 benchmark 融合模型）。** 横轴为打乱该特征后 macro-AUPRC 的下降（= 模型
...
训练集可观测阳性支持度 n_c⁺。最易预测的隔室为 [ ]、最难为 [ ]（图 8）。逐隔室“性能 ~ 支持度”的
Spearman ρ=[ ]（图 9）——若强相关，提示隔室间性能差异主要由可观测阳性数解释，而非该隔室无序列可
预测信号。逐隔室“融合 − k-mer”增益见图 10、表 6：[ ]/5 个隔室在 macro-AUPRC 下显著，融合对低支持度
隔室 [是否] 仍有增益。

![逐隔室性能](../results/figures/fig_fine_percompartment.png)

**图8. 主 benchmark 融合模型的逐隔室 AUPRC / AUROC（灰虚线 = 该标签先验）。**

![支持度 vs 性能](../results/figures/fig_fine_support_vs_perf.png)

**图9. 训练阳性支持度 n_c⁺ 与测试 AUPRC 的关系（Spearman ρ）。**

![逐隔室增益](../results/figures/fig_fine_percompartment_gain.png)

**图10. 逐隔室“融合 − k-mer”差值及 95% CI（左 AUPRC、右 AUROC，分组自助法）。**
...
的描述性指标见图 11（不做跨物种统计检验，仅描述）。**留一来源（去除最大主导来源后重训主模型一次）**
...
**图11. 各物种测试子集上的主 benchmark 融合性能（macro-AUPRC / macro-AUROC，描述性）。**
...
| 图5 融合 vs k-mer | §3.3 | `fig_fine_fusion_forest.*` |
| 图6 成分分解（+ 门控 `fig_fine_gate`） | §3.3 | `fig_fine_components.*` |
| 图7 工程特征置换重要性 | §3.3 | `fig_fine_feature_importance.*` |
| 图8 逐隔室性能 | §3.4 | `fig_fine_percompartment.*` |
| 图9 支持度 vs 性能 | §3.4 | `fig_fine_support_vs_perf.*` |
| 图10 逐隔室融合增益 | §3.4 | `fig_fine_percompartment_gain.*` |
| 图11 各物种性能 | §3.5 | `fig_fine_perspecies.*` |
