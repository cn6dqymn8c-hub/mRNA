...
def draw_funnel(ax):
    ...
    for y, (name, d) in zip(ypos, stages):
        ...
        for sp in SP_ORDER:
            ax.barh(y, d.get(sp, 0), left=left, color=C_SP[sp], edgecolor="white", height=0.62)
            left += d.get(sp, 0)
...
outer = fig.add_gridspec(2, 1, height_ratios=[1, 1.55], hspace=0.32)
top = outer[0].subgridspec(1, 3, wspace=0.34)
draw_funnel(fig.add_subplot(top[0]))
draw_species_compartment(fig.add_subplot(top[1]))
draw_length(fig.add_subplot(top[2]))
